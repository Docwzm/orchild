  ...
  <div class="contractSign">
    ...
    <div class="pdfContainer" style="top: 130px;">
      <img
        ref="contractPic"
        @load="imgOnLoad"
        :src="options.contractPicUrl"
        :style="{ width: '100%', height: contractZoomHeight + 'px' }"
      >
    </div>
    <div
    ...
    >
      <img
        class="seal"
        @load="sealOnLoad"
        :style="{width:sealSize.width+'px',height:sealSize.height+'px'}"
        :src="signInfo.sealUrl"
      >
    </div>
      ...
      <div class="signConfir">
        ...
        <div class="jx-dialog__content">
          ...
          <div class="verification" v-if="selfSign.signStatus != 12">
            <van-cell-group>
              <van-field readonly v-model="phone" center clearable label="手机号" placeholder="请输入手机号">
                <van-button
                  slot="button"
                  @click="sendCode"
                  type="primary"
                  color="linear-gradient(90deg, #ff9916 0%, #ffa81e 40%, #ffb424 66%, #ffc32c 100%)"
                >{{codeTxt}}</van-button>
              </van-field>
              <van-field v-model="messageCode" label="授权码" placeholder="请输入授权码"/>
            </van-cell-group>
          </div>
          <div class="checkInfo">
...
export default class ContractSign extends Vue {
    ...
    mounted() {
        ...
        this.containerWith =
            document.body.clientWidth || this.$refs.contractPic.clientWidth
        this.getContractLog()
    ...
    contractSignListHandle() {
        ...
        for (let item of this.options.contractSignList) {
            if (
                item.signerId == loginUserMemberId &&
                item.organizationId == loginUserOrgId &&
                item.signStatus != 15
            ) {
                this.selfSign = item
    ...
    getCompSignature() {
        ...
        ContractService.getCompSignature(params)
            .then(res => {
                that.$toast.clear()
                let _res: any = res
    ...
    getContractLog() {
        ...
        ContractService.getContractLog(params)
            .then(res => {
                that.$toast.clear()
